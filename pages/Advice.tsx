...
const Advice: React.FC = () => {
  ...
  const getNewAdvice = useCallback(() => {
    let newKey;
    if (ADVICE_KEYS.length === 0) return;
    if (ADVICE_KEYS.length === 1) {
        newKey = ADVICE_KEYS[0];
    } else {
        do {
            newKey = ADVICE_KEYS[Math.floor(Math.random() * ADVICE_KEYS.length)];
        ...
        <div className="flex flex-col items-center text-center p-4 min-h-[150px] justify-center">
          ...
          {currentAdviceKey ? (
            <div className="animate-fadeIn">
              <h2 className="text-xl font-bold mb-2">{t(`${currentAdviceKey}_title`)}</h2>
              <p className="text-gray-600 dark:text-gray-400">{t(`${currentAdviceKey}_text`)}</p>
            </div>
          ) : (
            <p className="text-gray-600 dark:text-gray-400">{t('click_button_for_advice')}</p>
          )}
